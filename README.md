...
- `data/sector_daily_scores.csv`：板块级每日快照；同日分别保存 `baseline` 和 `enhanced` 两个 `formula_version`，并用 `pipeline_revision` 标记语义修订。
...
- `pipeline_revision`：每日快照的数据管线语义修订号。历史行补标为 `r1`，本批风险公式与聚类护栏启用后写入 `r2`；趋势出现断点时可据此区分真实舆情变化与公式修订影响。
...
- Risk Intensity：未命中风险时为 0。每个命中类别先计算句级密度 `r_k = min(命中句子数 / 总句子数 * 3, 1)`，文章层再计算 `100 * clip(sum((v_k / 5) * r_k), 0, 1)`，支持分号分隔的多标签；情绪压力开关默认关闭，避免与 Fear/Uncertainty 耦合。板块层为 `0.7 * 加权平均 + 0.3 * P90`，少于 3 条时以均值替代 P90。
