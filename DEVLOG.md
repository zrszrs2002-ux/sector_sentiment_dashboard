...
- 阶段名称 / 本次操作目标：热力图 hover 数值修复（外部直接修改，非 Codex 会话产出）。
- 具体做了什么：热力图悬浮框自 Tier-A 引入 hovertemplate 起即显示"-"而非原始分——heatmap 逐格 customdata 在模板中解析为标量，`%{customdata[0]:.1f}` 对标量再索引得到 undefined。修复：`src/ui_helpers.py` 中模板改为 `%{customdata:.1f}`，仅此一处，其余渲染逻辑不变。
- 验证：浏览器内对三种模板写法实测对比（`%{customdata:.1f}` 与 `%{text}` 均正确显示 23.8，旧写法显示"-"）；修复当时 `python -m compileall` 通过、`python -m unittest discover -s tests` 全部通过；用户在本机 dashboard 人工验收通过。
- 当前项目状态：该修复代码已随 `9d0a80a` 提交，本条为事后补记。
